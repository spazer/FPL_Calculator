...
namespace WL_Calculator
{
    partial class Stats
    {
        ...
        /// FPL stats page
        /// </summary>
        const string FPLSTATS = "http://www.teamliquid.net/fantasy/proleague/Stats.php?r=13";
        ...
        #region Constructor
        public Stats(ErrorWriter ewriter, double inputPointModifier) 
        {
            errorwriter = ewriter;
            ...
            int tempindex = 0;
            int tableEnd = 0;
            pointModifier = inputPointModifier;
            ...
            int tableStart = source.IndexOf("<table class='highlight'>");
            index = tableStart;

            // Locate end of team table
            tableEnd = source.IndexOf("</table>", index);

            // Get # of columns between team and cost
            index = source.IndexOf("Team", index) + 1;
            int costRows = 0;
            do
            {
                index = source.IndexOf("<th", index);
                index = source.IndexOf("<a", index);
                tempindex = source.IndexOf(">", index) + 1;
                costRows++;
            } while (source.Substring(tempindex, 1) != "$");
            ...
            do
            {
                try
                {
                    ...
                    teamList.Add(name.ToUpper(), new Team(name, cost, ARRAYSIZE, TOTALGAMES, pointModifier));
                }
                catch (Exception e)
                {
            ...
            // Locate # of columns from team to cost stat
            index = source.IndexOf("Team", index);
            costRows = 0;
            do
            {
                index = source.IndexOf("<th", index) + 3;
                costRows++;
            } while (index < source.IndexOf("Cost", tableStart) && index != -1 + 3);
            ...
            do
            {
                try
                {
                    ...
                    // Skip columns to reach cost column
                    for (int i = 0; i < costRows - 1; i++)
                    {
                        index = source.IndexOf("<td", index) + 3;
                    }
                    ...
                    playerList.Add(name.ToUpper(), new Player(name, cost, race, team, ARRAYSIZE, TOTALGAMES, pointModifier));
                }
                catch (Exception e)
                {
        ...
        private void boxParser(string input, int week)
        {
            ...
            for (int i = 0; i < playerList.Count; i++)
            {
                ...
                else if (playerList.ElementAt(i).Value.Team == teamlose)
                {
                    playerList.ElementAt(i).Value.TeamLose(week);
                }
            }
            ...
            List<string> appearedAlready = new List<string>();
            int streak1 = 0;
            int streak2 = 0;
            ...
            {
                string gameString = "m" + game.ToString();

                // Get player 1
                index = input.IndexOf(gameString + "p1=", index);

                if (index == -1)
                {
                    errorwriter.Write("Game " + game + " not found");
                    break;
                }
                else
                {
                    index += gameString.Length + 3;
                }
                ...
                // Give appearance points if this is their first appearance
                if (!appearedAlready.Contains(player1))
                {
                    playerList[player1].AppearancePoints(week);
                    appearedAlready.Add(player1);
                }
                if (!appearedAlready.Contains(player2))
                {
                    playerList[player2].AppearancePoints(week);
                    appearedAlready.Add(player2);
                }

                // Allocate wins and losses
                if (winner == "1")
                {
                    playerList[player1].WinPoints(week, game, streak2);
                ...
                {
                    playerList[player2].WinPoints(week, game, streak1);
        ...
        public void PrintTeamTrVal()
        {
            StreamWriter output = new StreamWriter(".//TeamTrVal.txt");
            ...
            for (int i = 0; i < teamList.Count; i++)
            {
                ...
                for (int j = 0; j < ARRAYSIZE; j++)
                {
                    output.Write("\t" + teamList.ElementAt(i).Value.AdjustedTradeValue(j).ToString());
                }
        ...
        public void PrintPlayerTrVal()
        {
            StreamWriter output = new StreamWriter(".//PlayerTrVal.txt");
            ...
            for (int i = 0; i < playerList.Count; i++)
            {
                ...
                // Write weekly trade value
                for (int j = 0; j < ARRAYSIZE; j++)
                {
                    output.Write("\t" + playerList.ElementAt(i).Value.AdjustedTradeValue(j).ToString());
                }
        ...
        public void BestAntiGenerationWithTrades(int maxPlayers)
        {
            ...
            {
                writer.WriteLine("Week " + i);
                writer.WriteLine("Name\tCost\tTrVal\tPoints");
                for (int j = 0; j < 3; j++)
                {
                    writer.WriteLine(playerList[selectedPlayers[i - 1][j]].Name + "\t" +
                                     playerList[selectedPlayers[i - 1][j]].Cost + "\t" +
                                     playerList[selectedPlayers[i - 1][j]].AdjustedTradeValue(i) + "\t" +
                                     playerList[selectedPlayers[i - 1][j]].Points(i - 1));
                }
